...
class BetRecordV2:
    ...
    data_status: str = "clean"  # clean / live / result_exposed / proxy / audit
    kickoff_status: str = "pre_match"  # pre_match / live / post_match
    recommended_stake: Optional[float] = None
    odds_source: str = "unknown"
    is_official_roi: bool = False
    notes: str = ""
    error_tags: List[str] = field(default_factory=list)

    def settle(self, result: str, odds: Optional[float] = None) -> "BetRecordV2":
...
class LedgerV2:
    def __init__(self) -> None:
        self.records: List[BetRecordV2] = []

    def add(self, record: BetRecordV2) -> None:
        if record.book not in VALID_BOOKS_V2:
    ...
    def summary(self, include_books: Optional[List[str]] = None, official_only: bool = False) -> Dict[str, float]:
        rows = self.records
        if include_books is not None:
            rows = [r for r in rows if r.book in include_books]
        if official_only:
            rows = [r for r in rows if r.is_official_roi]
